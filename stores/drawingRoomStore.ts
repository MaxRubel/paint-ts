...
export function UnpackTransition(){
    ...
    if(id){
        fetched_single.set({id, name, owner, collaborators, date_created, data:{
            rectangles: {}, canvasImage: ""
        }})
    }
    fetched_single.update((preVal)=>({
        ...preVal, data: {rectangles: data.rectangles, canvasImage: data.canvasImage}
    }))
    ...
    const img = new Image();
        ...
        img.onload = function () {
            setTimeout(()=>{
                ...
                if(!ctx){
                    console.error('Oopsies-error transitioning to drawing room.  Canvas Context was not initialized when the canvas image attempted to load')
                    return
                }
